...
x_start = 2060
x_end = -1955   # We use 965 so the loop includes exactly 97.0
x_step = -5 

# Y goes from 115 up to 119 (+5 step)
y_start = 2025
y_end = 1985  # We use 1195 so the loop includes exactly 119
y_step = -5 
...
for x_val in range(x_start, x_end, x_step):
    for y_val in range(y_start, y_end, y_step):
        ...
        filename = f"Images_Pin/degree_img_x{x}_y{y}_z{z_height}.jpg"
        
        # Take the picture and save it
        img = tp.take_picture(filename, 0)
